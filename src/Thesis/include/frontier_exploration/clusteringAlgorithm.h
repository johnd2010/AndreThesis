...
class MSCluster : public MeanShift {
  public:
    void getMeanShiftClusters(vector<geometry_msgs::Point> &points, vector<geometry_msgs::Point> &cluster_points, double kernelBandwidth)
    {
      ...
      for (int i = 0 ; i < clusters.size(); i++)
      {
        ...
        // Find min element
        int minElementIndex = std::min_element(vectorOfDistances.begin(), vectorOfDistances.end()) - vectorOfDistances.begin();
