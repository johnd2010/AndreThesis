...
bag_path = '/home/andre/thesis/sims/3_uav/test3.bag'
...
data = {
    ...
    'time_coverage3': [], 'occupied3': [], 'free3': [], 'missing3': [], 'mapped3': [],
    'uavs_in_range1': [], 'uavs_in_range2': [], 'uavs_in_range3': [],
}
...
with rosbag.Bag(bag_path, 'r') as bag:
    ...
    prev_position2 = None
    prev_position3 = None
    for topic, msg, t in bag.read_messages(topics=['/uav1/estimation_manager/odom_main' ,'/uav2/estimation_manager/odom_main', '/uav3/estimation_manager/odom_main', '/uav1/coverage', '/uav2/coverage', '/uav3/coverage', '/uav1/uavs_in_range', '/uav2/uavs_in_range', '/uav3/uavs_in_range']):
        try:
            if topic == '/uav1/estimation_manager/odom_main':
                ...
                if prev_position1 is not None and data['time_odom'][-1] - data['time_odom'][0] < 86.7:
                ...
                if prev_position2 is not None and data['time_odom'][-1] - data['time_odom'][0] < 86.7:
                    total_distance2 += np.linalg.norm(curr_position2 - prev_position2)
                prev_position2 = curr_position2
            elif topic == '/uav3/estimation_manager/odom_main':
                data['time_odom'].append(t.to_sec())
                data['x3'].append(msg.pose.pose.position.x)
                data['y3'].append(msg.pose.pose.position.y)
                data['z3'].append(msg.pose.pose.position.z)
                # Calculate distance
                curr_position3 = np.array([msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z])
                if prev_position3 is not None and data['time_odom'][-1] - data['time_odom'][0] < 86.7:
                    total_distance3 += np.linalg.norm(curr_position3 - prev_position3)
                prev_position3 = curr_position3
            ...
            elif topic == '/uav3/coverage':
                if first_timestamp_coverage is None:
                    first_timestamp_coverage = msg.data[0]
                data['time_coverage3'].append(msg.data[0] - first_timestamp_coverage)
                data['occupied3'].append(100 * msg.data[1] / msg.data[4])
                data['free3'].append(100 * msg.data[2] / msg.data[4])
                data['mapped3'].append(100 * (msg.data[1] + msg.data[2]) / msg.data[4])
                data['missing3'].append(100 * msg.data[3] / msg.data[4])
            ...
            elif topic == '/uav3/uavs_in_range':
                data['uavs_in_range3'].extend(msg.data)
        except Exception as e:
            print("Failed to append data: ", e)

print(data['time_odom'][-1] - data['time_odom'][0])
...
print("Total distance travelled by UAV3: ", total_distance3, "meters")
...
print("Total time travelled by UAV3: ", data['time_coverage3'][-1], "seconds")

print("Total coverage by UAV1: ", data['mapped1'][-1], "percent")
print("Total coverage by UAV2: ", data['mapped2'][-1], "percent")
print("Total coverage by UAV3: ", data['mapped3'][-1], "percent")
...
data['x3'] = data['x3'][:min_length]
data['y3'] = data['y3'][:min_length]
data['z3'] = data['z3'][:min_length]

# Plot odometry data
ax.scatter(data['x1'], data['y1'], data['z1'], s=1)
ax.scatter(data['x2'], data['y2'], data['z2'], s=1)
ax.scatter(data['x3'], data['y3'], data['z3'], s=1)
...
ax2.plot(data['time_coverage3'], data['mapped3'], label='UAV3 Mapped Cells', linestyle='solid')
