...
private:
    ...
    void frontierDetection()
    {
        ...
        ROS_INFO_STREAM("Time elapsed (secs) Frontier detection : "<<detection_time.toSec());
    ...
    void frontierClustering()
    {
        ...
        ROS_INFO_STREAM("Time elapsed (secs) Clustering : "<<clustering_time.toSec());
    ...
    void frontierEvaluation()
    {
        ...
        ROS_INFO_STREAM("Time elapsed (secs) ViewpointGeneration + Best Viewpoint : "<<evaluation_time.toSec());
    ...
    void pathPlanner()
    {
        ...
        while((ros::WallTime::now() - update_time).toSec() < update_rate)
        {
            ...
            if(!planner_srv.call(srv))
                ROS_ERROR_ONCE("Failed to call path planner service for UAV %d", uav_id);
    ...
    void calculateCoverage() {
        ...
        double percentages_x = 100.0 * occupied_cells / total_cells;
        double percentages_y     = 100.0 * free_cells     / total_cells;
        double percentages_z  = 100.0 * (total_cells - (occupied_cells + free_cells))  / total_cells;

        ROS_INFO_STREAM("Occupied : "<<percentages_x<< " Free : "<< percentages_y<< " Unknown : "<< percentages_z << " Known : "<<percentages_x + percentages_y);
        if((percentages_x + percentages_y)>90)
